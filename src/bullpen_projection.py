"""Projected bullpen availability and matchup weighting for Advanced HVP."""
...
def availability_from_workload(
    ...
    if safe_float(workload.get("recent_innings_pitched")) and safe_float(workload.get("recent_innings_pitched")) >= 3:
...
def normalize_appearance_probabilities(relievers):
    ...
    total = sum(_normalization_weight(row["projected_role"], row["availability_score"]) for row in weighted)
    if total <= 0:
        for row in weighted:
...
def build_projected_bullpen(
    ...
    if team_id is not None and "team_id" in roster.columns:
        roster = roster[
            pd.to_numeric(roster["team_id"], errors="coerce").eq(int(team_id))
        ].copy()
...
def _projection_row(
    ...
    return {
        "player_id": player_id,
        "Player": _player_name(roster_row) if _player_name(roster_row) != "Pitcher" else _player_name(stats_row),
