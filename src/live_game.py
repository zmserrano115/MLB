...
def build_live_game_demo_feed(tick=0):
    """Isolated preview-only live state. Never used unless explicitly enabled."""
    phase = int(tick) % 5
    score = 2 if phase < 4 else 4
    strikes = (0, 1, 2, 0, 0)[phase]
    fouls = (0, 0, 1, 0, 0)[phase]
    batter = (
        {"id": 1, "fullName": "Demo Batter"}
        if phase < 3
        else {"id": 5, "fullName": "Next Demo Batter"}
    )
    runner = (
        {
            "player_id": 3,
            "name": "Demo Runner",
            "headshot": player_headshot_url(3),
        }
        if phase == 3
        else None
    )
    ...
    result_label = (
        PLAY_RESULT_LABELS[play_type]
        if play_type in PLAY_RESULT_LABELS
        else play_type.replace("_", " ").title()
    )
    description = {
        "ball": "Ball outside. The count updates live.",
        "foul": "Demo Batter fouls off a two-strike pitch.",
        "single": "Demo Batter singles to center field.",
        "home_run": "Next Demo Batter homers to left field.",
    }[play_type]
    hit_data = None
    if play_type == "single":
        hit_data = {
    ...
    demo_pitch_locations = [
        {"p_x": -1.22, "p_z": 3.58, "call": "Ball", "is_ball": True, "zone": 11},
        {"p_x": 0.18, "p_z": 2.42, "call": "Called Strike", "is_strike": True, "zone": 5},
        {"p_x": 0.58, "p_z": 2.18, "call": "Foul", "is_strike": True, "zone": 8},
        {"p_x": -0.22, "p_z": 2.74, "call": "In play", "is_in_play": True, "zone": 4},
        {"p_x": 0.34, "p_z": 2.83, "call": "In play, run(s)", "is_in_play": True, "zone": 3},
    ]
    demo_pitches = []
    for pitch_index, pitch in enumerate(demo_pitch_locations[: phase + 1], start=1):
        demo_pitches.append(
            {
                "play_id": f"demo-{pitch_index}",
                "event_index": pitch_index,
                "description": pitch["call"],
                "code": None,
                "call": pitch["call"],
                "is_strike": bool(pitch.get("is_strike")),
                "is_ball": bool(pitch.get("is_ball")),
                "is_in_play": bool(pitch.get("is_in_play")),
                "zone": pitch.get("zone"),
                "pitch_type": "Four-Seam Fastball",
                "pitch_code": "FF",
                "start_speed": 94.0 + pitch_index,
                "p_x": pitch.get("p_x"),
                "p_z": pitch.get("p_z"),
                "strike_zone_top": 3.5,
                "strike_zone_bottom": 1.55,
            }
        )
    current_batter = _live_player(batter)
    current_batter["lineup_number"] = 2 if phase < 3 else 3
    on_deck = _live_player({"id": 4, "fullName": "On Deck Hitter"})
    on_deck["lineup_number"] = 3 if phase < 3 else 4
    demo_play = {
        "play_index": phase,
        "completed": True,
        "inning": 6,
        "half_inning": "top",
        "batter": _live_player(batter),
        "pitcher": _live_player({"id": 2, "fullName": "Demo Pitcher"}),
        "result_type": play_type,
        "result_label": result_label,
        "description": description,
        "count_before": {"balls": 1, "strikes": strikes, "outs": 1},
        "count_after": {"balls": 1, "strikes": strikes, "outs": 1},
        "runs_scored": 2 if play_type == "home_run" else 0,
        "away_score": score,
        "home_score": 1,
        "is_scoring_play": play_type == "home_run",
        "hit_data": hit_data,
        "batting_side": "away",
    ...
    return {
        ...
        "abs_challenges": {
            "enabled": True,
            "away": {"remaining": 2, "successful": 0, "failed": 0},
            "home": {"remaining": 1, "successful": 1, "failed": 1},
        },
        "balls": 1,
        "strikes": strikes,
        "outs": 1,
        "fouls": fouls,
        "current_pitches": demo_pitches,
        "latest_pitch": demo_pitches[-1] if demo_pitches else None,
        "current_batter": current_batter,
        "current_pitcher": {
            **_live_player({"id": 2, "fullName": "Demo Pitcher"}),
            "pitch_count": 87,
            "strikeouts": 7,
            "hits_allowed": 4,
            "era": "2.84",
        ...
        "on_deck": on_deck,
        "bases": {"first": runner, "second": None, "third": None},
        "recent_plays": [demo_play],
        "contact_plays": [demo_play] if hit_data else [],
        "team_batting_summary": {
            "away": {
                "hits": 10 if phase >= 4 else 8,
                "at_bats": 34,
                "avg": ".294",
                "obp": ".351",
                "ops": ".812",
        ...
        "latest_completed_play": {
            "play_index": phase,
            "result_type": play_type,
        },
        "latest_batted_ball": demo_play if hit_data else None,
    }
